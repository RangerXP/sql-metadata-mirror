...
> see `docs/01_Notebook_Description.md` through `docs/10_Notebook_Description.md` (all 10 now
> split into one file per notebook; see `docs/notebook-legacy-reference.md` for the legacy name
> mapping table). The `sql/` folder referenced throughout was also
...
| Milestone M4 — MIP sensitivity labels | IN_PROGRESS | Live `setLabels` succeeded on `BrookfieldEnercare`; SQL, Purview Data Map, and Fabric Information Protection receipts passed. Item-wide DLP Restrict Access also blocked authorized users, so enforcement is moving to `RestrictedGeolocation` object-level security on `dim_service_account[Latitude]` and `[Longitude]`. Completion requires deployment, Rupal role assignment, removal of only the DLP Restrict Access action, and persona tests proving general model access for all six stakeholders while protected coordinates remain hidden from Rupal. |
