...
| Milestone M4 — MIP sensitivity labels | IN_PROGRESS | Live `setLabels` succeeded on `BrookfieldEnercare`; SQL, Purview Data Map, and Fabric Information Protection receipts passed. The owner-only DLP action also blocked Victoria, the authorized workflow approver, so it is not the final access design. Completion requires Fabric protection-policy allowlisting for Sean and Victoria, removal of the conflicting DLP Restrict Access action, and a positive Victoria / negative Rupal retest |
