...
**Recommendation:** Decide whether to fix the `nb_05` gap before the demo pass (low-risk,
single `WHERE` clause change + one seed-time value) or accept it as a known, documented gap for
this pass and track it as a new gap entry in `docs/design-gap-analysis.md`.
