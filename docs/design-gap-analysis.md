...
| G16-3 | P4 — Data product publish (`DP-SVCPERF`/DOM-SVCDEL): Ranbir Singh approver, Shruthi Srinivas requester | 🟡 In Progress — `nb_15` live evidence chain fully captured (2026-08-12): `request=PV-DP-SVCPERF-9EAF4919D7DFD8F8B5C6 status=Approved` with real Draft+Published object versions and a Passed `PublicationReadback` receipt, via a genuine live Publish→approve cycle by Ranbir Singh | Next: `nb_16` semantic reconciliation — `SEMANTIC_TARGETS` still need confirmation against the live semantic model before its first live run; request not yet marked Completed (that's nb_16's job) |
