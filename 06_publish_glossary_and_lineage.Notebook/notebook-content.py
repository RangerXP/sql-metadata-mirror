...
def _get_purview_token_via_tokenlibrary_early() -> str:
    ...
    # rely on via mssparkutils.credentials.getToken). No retry here: repeated
    # live runs show Spark_System_TM_INTERNAL_ERROR failing identically on
    # every attempt, not as an intermittent blip -- retrying doesn't help and
    # only wastes time. If this fails, set PURVIEW_ACCESS_TOKEN instead (see
    # _resolve_purview_token below, which tries that first).
    last_error = None
    for resource in ("https://purview.azure.net", "https://purview.azure.net/.default"):
        try:
            ...
            last_error = exc
            print(f"[AUTH][WARN] TokenLibrary failed for resource={resource}. Error: {exc}")
