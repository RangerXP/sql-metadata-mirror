...
Grounding is written at the **semantic-model level** as TOM **descriptions** (first-class) + **annotations** (`AI_Instruction`, `PBI_AI_Instructions`, `IsCertified`, `GlossaryTerm`, verified Q&A) via SemPy Labs (`nb_04_sempy_writeback`, `nb_05_push_qa_verified_answers`). They travel with the model (export, Git-TMDL sync, Purview scan) and are read by every consumer — **no `.copilot`/skills folder**. Optionally mirror key descriptions onto the lakehouse table (data plane) for OneLake-catalog discovery.
